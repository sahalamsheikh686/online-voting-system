        ...
        <div class="col-xl-6 col-lg-7">
            <div class="panel-card p-4 p-lg-5">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
                    ...
                    <a href="{{ route('register') }}" class="btn btn-outline-secondary rounded-pill px-4">Back to Register</a>
                </div>

                <form action="{{ route('register.otp.store') }}" method="POST" class="row g-4" autocomplete="off">
                    @csrf
                    <div class="col-12">
                        <label class="form-label">Email</label>
                        <input type="email" name="email" value="{{ old('email', $email) }}" class="form-control form-control-lg" placeholder="you@example.com" autocomplete="off">
                    </div>
                    <div class="col-12">
                    ...
                    <div class="col-12 d-flex gap-3">
                        <button class="btn btn-primary px-4">Verify &amp; Complete Registration</button>
                        <a href="{{ route('login') }}" class="btn btn-outline-secondary px-4">Cancel</a>
                    </div>
                </form>

                <form action="{{ route('register.otp.resend') }}" method="POST" class="mt-3">
                    @csrf
